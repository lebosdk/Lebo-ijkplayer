...
static int queue_picture(FFPlayer *ffp, AVFrame *src_frame, double pts, double duration, int64_t pos, int serial)
{
    ...
    /* lebo start: modify accurate seek */
    if (ffp->enable_accurate_seek && is->video_accurate_seek_req && !is->seek_req) {
        ...
        if (video_accurate_seek_fail) {
            av_log(NULL, AV_LOG_WARNING, "video accurate_seek is error, video_seek_pos=%lld, is->seek_pos=%lld, is->drop_vframe_count=%d, now=%lld, pts=%lf\n",
                video_seek_pos, is->seek_pos, is->drop_vframe_count, now, pts);
        } else {
            av_log(NULL, AV_LOG_INFO, "video accurate_seek is ok, video_seek_pos=%lld, is->seek_pos=%lld, is->drop_vframe_count=%d, now=%lld, pts=%lf\n",
                video_seek_pos, is->seek_pos, is->drop_vframe_count, now, pts);
        }
        ...
        if (video_seek_pos != -1 && video_seek_pos != is->seek_pos) {
            av_log(NULL, AV_LOG_INFO, "video accurate_seek new seek req, video_seek_pos=%lld, seek_pos=%lld.", video_seek_pos, is->seek_pos);
            is->video_accurate_seek_req = 1;
        }
        SDL_UnlockMutex(is->accurate_seek_mutex);
    }
    /* lebo end: modify accurate seek */
        ...
        if (got_frame) {
                ...
                if (ffp->enable_accurate_seek && is->audio_accurate_seek_req && !is->seek_req) {
                    ...
                    if (audio_accurate_seek_fail) {
                        av_log(NULL, AV_LOG_WARNING, "audio accurate_seek is error, audio_seek_pos=%lld, is->seek_pos=%lld, is->drop_aframe_count=%d, now=%lld, pts=%lf\n",
                            audio_seek_pos, is->seek_pos, is->drop_aframe_count, now, audio_clock);
                    } else {
                        av_log(NULL, AV_LOG_INFO, "audio accurate_seek is ok, audio_seek_pos=%lld, is->seek_pos=%lld, is->drop_aframe_count=%d, now=%lld, pts=%lf\n",
                            audio_seek_pos, is->seek_pos, is->drop_aframe_count, now, audio_clock);
                    }
                    ...
                    is->accurate_seek_start_time = 0;
                    audio_accurate_seek_fail = 0;
                    is->accurate_seek_aframe_pts = 0;
                    if (audio_seek_pos != -1 && audio_seek_pos != is->seek_pos) {
                        av_log(NULL, AV_LOG_INFO, "audio accurate_seek new seek req, audio_seek_pos=%lld, seek_pos=%lld.", audio_seek_pos, is->seek_pos);
...
static int read_thread(void *arg)
{
    ...
    /* lebo start: choose max resolution stream */
    int video_stream_count = 0;
    int max_resolution_stream = -1;
    int max_resolution_size = 0;
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        // choose max resolution stream, if has same resolution, prefer h264 stream
        if (type == AVMEDIA_TYPE_VIDEO) {
            enum AVCodecID codec_id = st->codecpar->codec_id;
            int resolution_size = st->codecpar->width * st->codecpar->height;
            video_stream_count++;
            if ((resolution_size > max_resolution_size)
                || (resolution_size == max_resolution_size && codec_id == AV_CODEC_ID_H264)) {
                max_resolution_size = resolution_size;
                max_resolution_stream = i;
            }
        }
    }
    if (video_stream_count > 1 && st_index[AVMEDIA_TYPE_VIDEO] < 0) {
        st_index[AVMEDIA_TYPE_VIDEO] = max_resolution_stream;
        av_log(NULL, AV_LOG_WARNING, "multiple video stream found, prefer max resolution stream: %d\n", max_resolution_stream);
    }
    /* lebo start: choose max resolution stream */
