...
static int recreate_format_l(JNIEnv *env, IJKFF_Pipenode *node)
{
    ...
    av_packet_unref(&opaque->sps_pps_pkt);
    /* lebo start: add support annex-b format extradata */
    for(int i = 0; i < opaque->codecpar->extradata_size; i+=4) {
        ALOGI("extradata[%d]: %02x%02x%02x%02x\n", (int)opaque->codecpar->extradata_size,
            (int)opaque->codecpar->extradata[i+0], (int)opaque->codecpar->extradata[i+1], (int)opaque->codecpar->extradata[i+2], (int)opaque->codecpar->extradata[i+3]);
    }
    if (opaque->codecpar->extradata && opaque->codecpar->extradata_size > 0) {
        if ((opaque->codecpar->codec_id == AV_CODEC_ID_H264 && opaque->codecpar->extradata[0] == 1)
            ...
            ALOGI("nal_size=%d\n", (int)opaque->nal_size);
            if (ffp->mediacodec_sps_pps_usage == MEDIACODEC_SPS_PPS_SET_CSD || ffp->mediacodec_sps_pps_usage == MEDIACODEC_SPS_PPS_SET_CSD_AND_FRAME) {
            ...
            if (ffp->mediacodec_sps_pps_usage == MEDIACODEC_SPS_PPS_SET_FRAME || ffp->mediacodec_sps_pps_usage == MEDIACODEC_SPS_PPS_SET_CSD_AND_FRAME) {
                if (!av_new_packet(&opaque->sps_pps_pkt, sps_pps_size)) {
                    memcpy(opaque->sps_pps_pkt.data, convert_buffer, sps_pps_size);
                }
            }
            free(convert_buffer);
#endif
        } else if (opaque->codecpar->extradata_size > 7
            && opaque->codecpar->extradata[0] == 0x0
            && opaque->codecpar->extradata[1] == 0x0
            && ((opaque->codecpar->extradata[2] == 0x1
                    && ((opaque->codecpar->codec_id == AV_CODEC_ID_H264 && (opaque->codecpar->extradata[3] & 0x1f) == 7)
                        || (opaque->codecpar->codec_id == AV_CODEC_ID_HEVC && ((opaque->codecpar->extradata[3] & 0x7e) >> 1) == 32)))
                || (opaque->codecpar->extradata[2] == 0x0
                    && opaque->codecpar->extradata[3] == 0x1
                    && ((opaque->codecpar->codec_id == AV_CODEC_ID_H264 && (opaque->codecpar->extradata[4] & 0x1f) == 7)
                        || (opaque->codecpar->codec_id == AV_CODEC_ID_HEVC && ((opaque->codecpar->extradata[4] & 0x7e) >> 1) == 32))))) {
            // annex-b format extradata
            opaque->nal_size = 0;
            if (opaque->codecpar->extradata[2] == 0x0) {
                ALOGI("extradata startcode is: 00 00 00 01\n");
                if (ffp->mediacodec_sps_pps_usage == MEDIACODEC_SPS_PPS_SET_CSD || ffp->mediacodec_sps_pps_usage == MEDIACODEC_SPS_PPS_SET_CSD_AND_FRAME) {
                    SDL_AMediaFormat_setBuffer(opaque->input_aformat, "csd-0", opaque->codecpar->extradata, opaque->codecpar->extradata_size);
                }
                if (ffp->mediacodec_sps_pps_usage == MEDIACODEC_SPS_PPS_SET_FRAME || ffp->mediacodec_sps_pps_usage == MEDIACODEC_SPS_PPS_SET_CSD_AND_FRAME) {
                    if (!av_new_packet(&opaque->sps_pps_pkt, opaque->codecpar->extradata_size)) {
                        memcpy(opaque->sps_pps_pkt.data, opaque->codecpar->extradata, opaque->codecpar->extradata_size);
                    }
                }
            } else {
                ALOGI("extradata startcode is: 00 00 01\n");
                size_t convert_size = opaque->codecpar->extradata_size + 1;
                uint8_t *convert_buffer = (uint8_t *)calloc(1, convert_size);
                if (!convert_buffer) {
                    ALOGE("%s:sps_pps_buffer: alloc failed\n", __func__);
                    goto fail;
                }
                convert_buffer[0] = 0x0;
                memcpy(convert_buffer + 1, opaque->codecpar->extradata, opaque->codecpar->extradata_size);
                if (ffp->mediacodec_sps_pps_usage == MEDIACODEC_SPS_PPS_SET_CSD || ffp->mediacodec_sps_pps_usage == MEDIACODEC_SPS_PPS_SET_CSD_AND_FRAME) {
                    SDL_AMediaFormat_setBuffer(opaque->input_aformat, "csd-0", convert_buffer, convert_size);
                }
                if (ffp->mediacodec_sps_pps_usage == MEDIACODEC_SPS_PPS_SET_FRAME || ffp->mediacodec_sps_pps_usage == MEDIACODEC_SPS_PPS_SET_CSD_AND_FRAME) {
                    if (!av_new_packet(&opaque->sps_pps_pkt, convert_size)) {
                        memcpy(opaque->sps_pps_pkt.data, convert_buffer, convert_size);
                    }
                }
                free(convert_buffer);
            }
    ...
    /* lebo end: add support annex-b format extradata */
...
static int input_sps_pps_l(IJKFF_Pipenode_Opaque *opaque)
{
    if (opaque->sps_pps_pkt.size <= 0 || !opaque->sps_pps_pkt.data) {
        return 0;
    }
    int ret = 0;
    ...
    ssize_t copy_size = 0;
    uint32_t queue_flags = 2;
    int64_t time_stamp = 0;

    ALOGI("%s: start\n", __func__);
    input_buffer_index = SDL_AMediaCodec_dequeueInputBuffer(opaque->acodec, AMC_INPUT_TIMEOUT_US * 2);
    if (input_buffer_index < 0) {
        ALOGE("%s: SDL_AMediaCodec_dequeueInputBuffer faild\n", __func__);
        ret = -1;
        goto fail;
    } else {
        copy_size = SDL_AMediaCodec_writeInputData(opaque->acodec, input_buffer_index, opaque->sps_pps_pkt.data, opaque->sps_pps_pkt.size);
        if (!copy_size) {
            ALOGE("%s: SDL_AMediaCodec_writeInputData failed\n", __func__);
            ret = -1;
            goto fail;
        }
        if (copy_size != opaque->sps_pps_pkt.size) {
            ALOGW("%s: size=%d, copy_size=%d\n", __func__, (int)opaque->sps_pps_pkt.size, (int)copy_size);
        }
    }
...
static int feed_input_buffer2(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *enqueue_count)
{
    ...
    if (d->pkt_temp.data) {
        ...
        if (input_buffer_index < 0) {
            ...
            if (copy_size != d->pkt_temp.size) {
                ALOGW("%s: size=%d, copy_size=%d\n", __func__, (int)d->pkt_temp.size, (int)copy_size);
            }
        }
...
static int drain_output_buffer_l(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *dequeue_count, AVFrame *frame, int *got_frame)
{
    ...
    if (output_buffer_index == AMEDIACODEC__INFO_OUTPUT_BUFFERS_CHANGED) {
        ...
        AMCTRACE("output_buffer_index=%d\n", (int)output_buffer_index);
...
int ffpipenode_config_from_android_mediacodec(FFPlayer *ffp, IJKFF_Pipeline *pipeline, SDL_Vout *vout, IJKFF_Pipenode *node) {
    ...
    case AV_CODEC_ID_H264:
        ...
        /* lebo start: donot judge profile */
        switch (opaque->codecpar->profile) {
            ...
            case FF_PROFILE_H264_HIGH_10:
                ALOGW("%s: MediaCodec: H264_HIGH_10: enabled\n", __func__);
                break;
            case FF_PROFILE_H264_HIGH_10_INTRA:
                ALOGW("%s: MediaCodec: H264_HIGH_10_INTRA: enabled\n", __func__);
                break;
            case FF_PROFILE_H264_HIGH_422:
                ALOGW("%s: MediaCodec: H264_HIGH_10_422: enabled\n", __func__);
                break;
            case FF_PROFILE_H264_HIGH_422_INTRA:
                ALOGW("%s: MediaCodec: H264_HIGH_10_INTRA: enabled\n", __func__);
                break;
            case FF_PROFILE_H264_HIGH_444:
                ALOGW("%s: MediaCodec: H264_HIGH_10_444: enabled\n", __func__);
                break;
            case FF_PROFILE_H264_HIGH_444_PREDICTIVE:
                ALOGW("%s: MediaCodec: H264_HIGH_444_PREDICTIVE: enabled\n", __func__);
                break;
            case FF_PROFILE_H264_HIGH_444_INTRA:
                ALOGW("%s: MediaCodec: H264_HIGH_444_INTRA: enabled\n", __func__);
                break;
            case FF_PROFILE_H264_CAVLC_444:
                ALOGW("%s: MediaCodec: H264_CAVLC_444: enabled\n", __func__);
                break;
            default:
                // maybe not get profile with avformat_find_stream_info when probesize or analyzeduration is too small
                ALOGW("%s: MediaCodec: (%d) unknown profile: enabled\n", __func__, opaque->codecpar->profile);
                break;
        }
        /* lebo end: donot judge profile */
