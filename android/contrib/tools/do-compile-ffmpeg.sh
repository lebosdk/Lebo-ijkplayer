...
$CC -lm -lz -shared --sysroot=$FF_SYSROOT -Wl,--no-undefined -Wl,-z,noexecstack $FF_EXTRA_LDFLAGS \
    -Wl,-soname,liblbffmpeg.so \
    $FF_C_OBJ_FILES \
    $FF_ASM_OBJ_FILES \
    $FF_DEP_LIBS \
    -o $FF_PREFIX/liblbffmpeg.so
...
ln -s $FF_PREFIX/liblbffmpeg.so $FF_PREFIX/shared/lib/liblbffmpeg.so
...
for f in $FF_PREFIX/lib/pkgconfig/*.pc; do
    ...
    mysedi $f 's/-lavcodec/-llbffmpeg/g'
    mysedi $f 's/-lavfilter/-llbffmpeg/g'
    mysedi $f 's/-lavformat/-llbffmpeg/g'
    mysedi $f 's/-lavutil/-llbffmpeg/g'
    mysedi $f 's/-lswresample/-llbffmpeg/g'
    mysedi $f 's/-lswscale/-llbffmpeg/g'
done
